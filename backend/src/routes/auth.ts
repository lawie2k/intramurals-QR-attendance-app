...
router.post('/signup', async (req, res) => {
  try {
    const { email, studentId, password } = req.body as { email: string; studentId: string; password: string };
    if (!email || !studentId || !password) return res.status(400).json({ error: 'Missing fields' });
